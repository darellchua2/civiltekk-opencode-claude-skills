...
markitdown-mcp's installer probe (setup.sh `install_markitdown_mcp`,
setup.ps1 `Install-MarkitdownMcp`) originally passed for any version while
the docs promised `markitdown-mcp==0.0.1a7` — a future 0.0.1a8 fix would
silently never reach deployed machines. Fix: include the version in the
probe (`pip show markitdown-mcp | grep -qF "Version: 0.0.1a7"`; ps1:
`$pipShow -like "*Version: 0.0.1a7*"`).
