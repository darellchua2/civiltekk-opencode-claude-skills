...
6. **Deploy-script comment literals** — `deploy/setup.sh` (search-anchor: `run_skill_profile` header comments) and `deploy/setup.ps1` (search-anchor: `Invoke-SkillProfile` header comments) hardcode "N primary-visible skills" / "N-allow allowlist" in prose comments above the profile functions (counts auto-derive functionally; the comments drift; line numbers drift too — search the anchors, don't trust cited lines). Missed by PLAN-404 rev 2 — caught at code review.
