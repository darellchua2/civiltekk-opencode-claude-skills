...
- `plugins/vibeguard.ts:78-80` resolves config by search path, not relative to the plugin file — the move to `plugins/vibeguard.config.json` only works because (a) Dockerfile:56 COPYs it to `/app/.opencode/vibeguard.config.json`, (b) setup.sh/ps1 deploy it to `~/.config/opencode/`, and (c) the local pm2 runtime reads it through the `opencode_app/.opencode/vibeguard.config.json` typechange symlink. Dropping any one of the three silently disables masking in that runtime.
