...
- **code-review-subagent**: PASS (final, 2026-09-26) — helper-invoked composition deterministic and formula-pinned (`tests/agent_lcd_pilot.bats` #10); seam-arg equivalence verified (init.mjs:907 `AGENTS_SRC`=repo/agents vs resolve-models.mjs:411 `O.agentsSrc` ← setup.sh:97); token gate green; core retains checklist, rubric, direct-caller gate, return contract, ponytail lens, voice; opencode prose fully restored via overlay
- **image-analyzer-subagent**: PASS (final, 2026-09-26) — pin, auth.json expectation, permission-rule delegability restored via overlay; fallback recipe intact in core; description de-pin is the single intentional registry diff (4.1)
- **requirements-specialist-subagent**: PASS (final, 2026-09-26) — headless clause generalized semantics-preserving; question-deny explanation and Task syntax restored via overlay; modes A/B/R, routing tree, workflow intact
