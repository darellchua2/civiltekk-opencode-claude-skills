...
- [x] **1.2** Add `tests/test_question_repair_plugin.test.ts` — `node:test` + `assert/strict` covering: description filled from label; label filled from first 5 words of description; question filled from header (and header from question, truncated to 30 chars); options missing both fields dropped; options-less items kept with `options: []`; items lacking both question and header dropped; all-items-dropped and non-object inputs returned as the original reference; valid payloads returned as the same reference (no mutation); truncating fills leave `__VG_…__` vibeguard placeholders intact (copy verbatim, never split); `multiple` defaulted to `false`; hook fires only for `event.tool === 'question'` (fake ctx capturing the hook)
    ...
    — **Done:** 11/11 green incl. audited fixture, placeholder-safety, no-mutation, hook-guard; files: tests/test_question_repair_plugin.test.ts; fixes: review round 1 rewrote the options-less test to keep-with-[] and added the neither-question-nor-header drop test (12/12 at d9bbbc8)
...
- Drop rules (amended per code-review round 1 + Mode R relay round 2): option entries missing BOTH `label` and `description`; question items that are not objects, or have no usable `question` AND `header`. Items whose `options` array is absent or empties after option repair are KEPT with `options: []` — safe under both schema readings (present-but-empty passes a required-check; the tool's render path auto-appends a free-text option regardless of option count); drop would risk silently losing a real question.
...
- GATE d9bbbc8 lint=n.a typecheck=n.a build=n.a unit=t e2e=n.a — review round 1 fixes; node --test 12/12, bats 351/351
