...
**Review round 1 (code-review-subagent, 0 BLOCK / 2 WARN / 4 NOTE):** WARN-1 fixed — setup.ps1 Remove-Item now $DryRun-guarded (preview prints "[DRY-RUN] Would remove"); bash leg stays run_cmd-wrapped (dry-run safe by construction). WARN-2 fixed — all 7 stale old-name refs swept (deploy/.AGENTS.md:61 — live deployed surface, ATTRIBUTION.md:47, learnings README:43, 3 plugin header self-refs, setup.ps1:2260 comment). NOTEs applied — learnings-autoinject.README.md added to both shim lists; plugins/README.md rule-4 filter list now includes ATTRIBUTION.md (MIT texts) and "never" reworded to convention-until-filter; shim bash loop simplified to exact stale filenames. NOTEs deferred — shim bats case (needs a CONFIG_DIR harness; shim is rm-only, dry-run-printed) + research/ left historical. Requirements Gaps: none.
GATE 9ce594a lint=n.a. typecheck=n.a. build=n.a. unit=t e2e=n.a. — post-review: 96 bats ok, node --test green
**Review round 2 (re-review): round-1 ps1 fix silently broke the legacy list (dropped .ts suffixes — 4/5 removals no-ops on Windows) and 2 stale refs survived the sweep. Round-2 fixes: ps1 list restored to exact basenames mirroring setup.sh:3244; .env.example:37 + docker-entrypoint.sh:111 stale refs swept. All round-1 fixes re-verified by reviewer. Requirements Gaps: none. Deferred: shim bats harness (derive ps1 expectations from setup.sh per derived-consistency-pins when it lands).
GATE 04e02b3 lint=n.a. typecheck=n.a. build=n.a. unit=t e2e=n.a. — post-round-2: 96 bats ok, node --test green, ps1 list mirrors setup.sh
