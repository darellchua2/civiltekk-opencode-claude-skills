...
- [ ] **1.2** `--list-items`: LIST_ITEMS single-step plan mode (critical `dump_catalog` step printing skills/agents ids grouped by category/tier from registry.json via node; packs + plugins names) + completion
...
- [ ] **2.2** Slim help regeneration: remove the demoted auto-update option rows, add the subcommand block + `--list-items`/`--preset`/`--save-preset` rows to both help surfaces; keep every existing row (aliases unchanged)
...
- [ ] **3.2** Full gate: `bash -n`, `bats tests/`, `node --test tests/*.test.ts`, ps1 structural pin suite green; diff scope = setup.sh, setup.ps1, README, new test
    — **Why:** Gate contract.
    — **Done when:** all green.
