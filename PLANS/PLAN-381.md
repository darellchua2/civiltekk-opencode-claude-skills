...
- [ ] `bats tests/` green (all 11 suites touching paths updated)
- [ ] `node deploy/init.mjs --list skills` works and lists 149 skills; `--list agents` lists 34
...
- [ ] `docker build` succeeds (or documented manual note if Docker unavailable)
- [ ] Grep gate: `grep -rn "opencode_app/.opencode" deploy/ tests/ .releaserc.json .github/workflows/release.yml README.md AGENTS.md opencode_app/ restart-opencode-pm2.sh` returns no hits (the four tracked symlinks under `opencode_app/.opencode/` are the sole sanctioned exceptions — they ARE that path; gate excludes symlink entries themselves)
...
| `deploy/setup.sh` | moved dirs, build-registry counts | user-space deploy (4,583 lines) | high |
| `deploy/setup.ps1` | same | Windows mirror | high |
...
- [ ] **1.2** Create tracked symlinks for local runtime: `opencode_app/.opencode/agents → ../../agents`, `…/skills → ../../skills`, `…/plugins → ../../plugins`, `…/vibeguard.config.json → ../../plugins/vibeguard.config.json`
    ...
    — **Consumers affected:** restart-opencode-pm2.sh (local pm2 serve); none in CI/Docker (excluded in 4.3).
...
- [ ] **2.1** `deploy/source.mjs` lines 21–22: `skillDir`/`agentDir` → `join(root, "skills")` / `join(root, "agents")`
    ...
    — **Done when:** `grep -n 'opencode_app' deploy/source.mjs` returns nothing; `node -e 'import("./deploy/source.mjs").then(m=>console.log(typeof m.listSkills))'` loads clean.
...
- [ ] **2.4** `deploy/build-site.mjs` line 18: `GH_BASE` → `https://github.com/darellchua2/opencode-config-template/blob/main` and downstream path joins updated to `skills/<name>/SKILL.md` / `agents/<stem>.md`
    ...
    — **Done when:** `node deploy/build-site.mjs` (or its dry mode) emits URLs containing `/blob/main/skills/`.
...
- [ ] **3.1** `deploy/setup.sh`: update all 23 `opencode_app` refs to the new layout (path vars, copy loops, counts/drift checks, help text; `opencode_app/opencode.json` refs stay)
    — **Why:** The user-space deploy is the primary consumer of the content dirs; 4,583 lines means refs hide in heredocs — edit by grep iteration, not by eyeballing.
    — **Done when:** `grep -n 'opencode_app/.opencode' deploy/setup.sh` returns nothing; `bash -n deploy/setup.sh` passes.
    — **Consumers affected:** every user-space deploy; bats count/drift suites.
- [ ] **3.2** `deploy/setup.ps1`: same for all 16 refs (Windows mirror)
    — **Why:** Mirror must not drift from setup.sh.
    — **Done when:** `grep -n 'opencode_app/.opencode' deploy/setup.ps1` returns nothing.
...
- [ ] **4.1** `.releaserc.json` asset globs: `opencode_app/.opencode/agents/**/*` → `agents/**/*`; `opencode_app/.opencode/skills/**/*` → `skills/**/*` (keep the adjacent `deploy/*.mjs` + `opencode_app/opencode.json` entries)
    ...
    — **Done when:** `grep -n 'opencode_app/.opencode' .releaserc.json` returns nothing; `npx semantic-release --dry-run` verified in CI (or noted).
...
- [ ] **5.1** Update all 11 bats files referencing old paths (`tests/init.bats`, `skill_profiles.bats`, `test_autoresearch_protocol.bats`, `test_autoresearch_skills.bats`, `test_count_drift.bats`, `test_default_behavior.bats`, `test_docling_skill.bats`, `test_markitdown_skill.bats`, `test_mcp_count_consistency.bats`, `test_pack_permissions.bats`, + any helper) to the new layout
    — **Why:** Tests encode the old layout as truth; they must encode the new one.
    — **Done when:** `grep -rn 'opencode_app/.opencode' tests/` returns nothing; `bats tests/` green.
...
- [ ] **6.1** Update in-content path references: `agents/opencode-tooling-subagent.md` + the 5 SKILL.md files (`agent-introspection-debugging-skill`, `context-budget-skill`, `documentation-consistency-skill`, `markitdown-mcp-skill`, `opencode-repo-setup-skill`) — replace `opencode_app/.opencode/…` mentions with the new root paths
    — **Why:** These files describe their own repo locations; stale paths misdirect maintainers and agents.
    — **Done when:** `grep -rn 'opencode_app/.opencode' agents/ skills/ plugins/` returns nothing.
    — **Consumers affected:** skill/agent readers; none runtime.

### Phase 7: Docs + final verification gates
- [ ] **7.1** Docs: README structure tree + install/npx sections; root `AGENTS.md` (§Source of Truth → root `skills/`+`agents/`, §Subagent Locations row → `agents/*.md`, §Secret Masking vibeguard path → `plugins/vibeguard.config.json`); `opencode_app/README.md` (Docker content sourcing note; note `opencode_app/.dockerignore` is inert since context=root); `THIRD_PARTY_LICENSES.md` line 203 gsap paths
    — **Why:** Docs are the map for every future contributor; the sync rules in AGENTS.md require doc updates with structural change.
    — **Done when:** `grep -rn 'opencode_app/.opencode' README.md AGENTS.md opencode_app/README.md THIRD_PARTY_LICENSES.md` returns nothing; README tree shows `skills/ agents/ plugins/` at root.
    — **Consumers affected:** humans, opencode-tooling-subagent doc-sync flows.
- [ ] **7.2** Final gates: (a) `bats tests/` green; (b) `node deploy/init.mjs --list skills | wc -l` = 149 and `--list agents` = 34; (c) `node deploy/init.mjs add tdd-subagent --dry-run` succeeds; (d) grep gate from Acceptance Criteria returns clean; (e) `npm pack --dry-run 2>&1 | grep -q 'skills/' && … | grep -q 'agents/'` passes; (f) `docker build` (best-effort — record note if unavailable)
    — **Why:** These are the ticket's acceptance criteria, executed.
    — **Done when:** All sub-gates pass (or docker noted unavailable) and results recorded in the PR body.
