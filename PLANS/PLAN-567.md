...
- [x] **1.1** In `installer/init.mjs` `cmdAdd`: first statement (before the `--all` branch) — `if (opts.prune) die("'--prune' is not an add flag — it belongs to the preset/init flow (set replace) and prune-only mode. 'add' already migrates legacy copies of the names it installs; to remove entries use 'remove' (user scope) or the preset flow with --prune.", 2);`
    ...
    — **Done:** guard at top of cmdAdd (covers --all + named, both scopes), die exit 2 with preset/prune-only/remove pointers; files: installer/init.mjs; fixes: none
- [x] **1.2** Help text: the `--prune` USAGE line (:1684) gains "(preset flow / prune-only; add rejects it)".
    — **Done when:** `--help` shows the note.
    — **Why:** undocumented rejection is a support burden.
    — **Consumers affected:** CLI users.
    — **Done:** --prune USAGE line now says "(preset flow / prune-only; add rejects it)"; files: installer/init.mjs; fixes: none
- [x] **1.3** Tests in `tests/init.bats`: three rejection invocations (user scope, project scope, `--all`) each asserting non-zero exit + message fragment in output + no install dir created. Existing prune-only test (:320) must stay green.
    ...
    — **Done:** one test pinning all three rejection paths (user, project, --all) incl. no-install negative assert; files: tests/init.bats; fixes: none
- [x] **1.4** Gate: `bats tests/init.bats` green.
    — **Done when:** exit 0.
    — **Why:** behavioral proof before the exit gate.
    — **Consumers affected:** Phase 2 gate.
    — **Done:** bats tests/init.bats → 42 ok / 0 not ok, exit 0 (prune-only suite unchanged); files: none; fixes: none
