...
GATE (fix-round 2 head) lint=- typecheck=- build=- unit=t e2e=n.a.  (bash -n ok; bats 477 ok / 0 fail — 13 select-items pins; node --test 30 pass / 0 fail; round-2 review: 4 WARN fixed — leak-test teeth (untouched-config assertions), bash-3.2-safe dry_args scalar form, vibeguard companion copy + .ts plugin filter, plugin-copy rc tracked into consume-once; --defaults packs mirror main default (Mode R); MCP_TO_PACK derived-coverage pin; scope = package.json/lock + deploy/tui.mjs + installer/deploy-plan-items.mjs + deploy/setup.sh + deploy/setup.ps1 + tests/test_select_items.bats)
