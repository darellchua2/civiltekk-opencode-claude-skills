...
GATE (fix-round head 2) lint=- typecheck=- build=- unit=t e2e=n.a.  (bash -n ok; bats 477 ok / 0 fail — 12 select-items pins after fix round; node --test 30 pass / 0 fail; code review round 1: 1 BLOCK fixed — select+dry-run forwarded init.mjs's own --dry-run (child CLIs inherit nothing) + leak net added; 6 WARN fixed — provenance closure-membership + two-source pin, plugins/implied-MCP consumption, unlink-on-success, validator registration, linear empty→skip, pin repairs (negated greps, non-empty equivalence, two-source attribution); npm ls resolves 0.5.11; scope = package.json/lock + deploy/tui.mjs + installer/deploy-plan-items.mjs + deploy/setup.sh + deploy/setup.ps1 + tests/test_select_items.bats)
