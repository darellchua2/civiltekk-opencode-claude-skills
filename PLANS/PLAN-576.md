...
- [x] **3.1** Create `installer/overlay.mjs` exporting a shared compose helper (read `agents/overlays/<stem>.<target>.md` when it exists, append blank-line-separated to the body; never compose for `agentMode: "verbatim"` or targets without a TARGETS row) and wire BOTH write paths: the `installer/init.mjs` agent write loop (after per-target frontmatter transform) and `resolve-models.mjs` `renderAgent` (compose before `injectModel`)
    ...
    — **Done:** installer/overlay.mjs created (composeAgentBody: overlay append, verbatim/target-row guards); wired into init.mjs agent loop (after agentMode transform) and resolve-models.mjs renderAgent (before injectModel); files: installer/overlay.mjs, installer/init.mjs, installer/resolve-models.mjs; fixes: none
- [x] **3.2** Align `tests/init.bats` agent enumeration with registry semantics (`-maxdepth 1` on the disk-side count, matching `build-registry.mjs` non-recursive enumeration), framed as semantics-preserving: overlays are not agents
