...
- [ ] **1.3** Wire `kilo-translate` into the user-scope install loop, `cmdUpdate` would-content, **and the project branch's agent write** — `writeInstall`'s `if (ocProject) … else kimiAgentContent` dispatch (the #454 code, `init.mjs:427-434`) must dispatch on `pCfg.agentMode` exactly like the user-scope loop; kimi behavior stays pinned by `kimi_target.bats:101`. Hash = `sha256Hex(translated content)` everywhere.
