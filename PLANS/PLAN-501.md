...
## Acceptance Criteria
- [ ] Trap invocation nounset-safe: `"${BASH_LINENO[0]:-0}"` (no behavior change for real unguarded errors in `main` execution — still logs + exits non-zero)
- [ ] The repro command exits 1 with no `unbound variable` on stderr
- [ ] `bats tests/` runs with zero BW01 warnings from `test_subcommands.bats`
- [ ] Regression pin: new bats test asserting the sourced-context failing call yields rc 1 (not 127) and no `unbound variable` output
...
- [ ] **1.2** Add `tests/test_err_trap_nounset.bats` with one house-idiom test pinning the repro: sourced setup.sh + bare failing call → rc exactly 1 (not 127) and no `unbound variable` in combined output.
