...
| `skills/ticket-creation-skill/SKILL.md` | jira-git-integration guard (1.1) | `wayfinder-skill:133`, `worktree-pipeline-skill:73` (re-pointed 1.6), internal self-ref `:192` (§MCP Availability Guard REST fallback), `agents/pr-workflow-subagent.md:122` (§Attribution pointer), `opencode-repo-setup-skill:60,140` | medium |
...
Cross-module consumers exist (agent + 4 non-target skills point into the edited nodes) → architecture review selected at pipeline Step 7. No frontend signal → uiux not selected.
...
- [ ] **1.1** Rewrite `skills/jira-git-integration-skill/SKILL.md` §MCP Availability Guard as the canonical policy block: disabled-by-default status, absent-tools rule (never attempt/hallucinate), per-project enable via `opencode-repo-setup-skill`, REST-fallback pattern with cloudId discovery (`_edge/tenant_info`), headless degrade order
    ...
    — **Done when:** the section carries the full policy incl. the cloudId discovery line, and `grep -rn '_edge/tenant_info' skills/` (excluding `_archived`) returns exactly this one file
...
- [ ] **1.2** `skills/git-issue-updater-skill/SKILL.md`: replace the guard body's policy bullets with one pointer line to `jira-git-integration-skill` §MCP Availability Guard + keep only the skill-specific REST comment endpoint (`POST .../issue/<KEY>/comment`); heading `## MCP Availability Guard (JIRA branch only)` stays verbatim
    ...
    — **Done when:** policy bullets (absent-tools rule, enable offer, cloudId) are gone from this file; heading + endpoint line remain
...
- [ ] **1.3** `skills/jira-status-updater-skill/SKILL.md`: same reduction as 1.2, keeping its REST transition endpoint (`POST .../transitions`); heading `## MCP Availability Guard` stays verbatim
    ...
    — **Done when:** policy bullets gone; heading + endpoint line remain; step 5's inline REST fallback note still coherent (it names the payload shape, not the guard)
...
- [ ] **1.4** `skills/ticket-creation-skill/SKILL.md`: reduce §MCP Availability Guard (JIRA steps) to the pointer + its REST fallback line; the three-option degrade ladder moves out (policy now lives at the home); internal reference at `:192` ("§MCP Availability Guard REST fallback") must still read true
    ...
    — **Done when:** guard section is pointer + endpoint only; `:192` reference resolves; heading stays verbatim
...
- [ ] **1.6** Re-point `skills/wayfinder-skill/SKILL.md:132-133` and `skills/worktree-pipeline-skill/SKILL.md:73` directly at `jira-git-integration-skill` §MCP Availability Guard (kill the pointer-to-pointer chain via ticket-creation)
    ...
    — **Done when:** both files name jira-git-integration-skill §MCP Availability Guard; `grep -rn 'ticket-creation-skill.*MCP Availability Guard' skills/` returns no policy-route hits
...
- [ ] **3.1** `skills/ticket-creation-skill/SKILL.md`: keep the §Prerequisites pointer sentence as the single statement; §Common Issues "GitHub CLI Missing or Not Authenticated" drops the duplicated explanation and the inline `gh auth login && gh auth status` block, keeping only the load-`gh-cli-setup-skill` pointer; no other change to the Example Usage section (rule-added-example-stale sweep: confirm the example teaches no gh-auth inline flow)
    — **Why:** AC3 — `gh-cli-setup-skill` owns install/auth; two restatements in one file is the drift the ticket exists to remove
    — **Done when:** exactly one fallback statement in the file; `grep -n 'gh auth login' skills/ticket-creation-skill/SKILL.md` returns nothing; example section still coherent
...
- [ ] **4.1** Pointer-resolution audit: `grep -rn '§MCP Availability Guard\|§Attribution' skills/ agents/` — every reference's target heading exists verbatim in the target file (heading-rename-syncs-quoted-pointers); plus `grep -rn '_edge/tenant_info' skills/` (excluding `_archived/`) returns exactly one hit (jira-git-integration)
    — **Why:** AC2's "pointers still resolve" is checkable only by sweeping every quoted § pointer repo-wide
    — **Done when:** audit prints zero broken references; single tenant_info hit confirmed
