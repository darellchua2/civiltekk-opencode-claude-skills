...
- [x] Step 10 Task prompt instruction in `worktree-pipeline-skill` switched to pipeline mode: skip pr-workflow steps 2 / 2.5 / 3 / 4 — gates verified by run-plan, docstrings filled per-phase, PLAN ticked+committed; `gh pr checks --watch` is the merge gate; agent's job reduces to PR create (`Closes <TICKET_ID>`) + JIRA link. (Deviation from ticket AC as written: +step 3, adopted Mode R round 2 — badges would mutate README after Step 9 review)
- [x] `pr-workflow-subagent.md` gains a pipeline-mode conditional (parent states gates green → skip 2 / 2.5 / 3 / 4, proceed to PR creation) so the skip is agent-side, not prompt-only; standalone behavior unchanged
...
- [x] **1.1** Add the pipeline-mode conditional to `agents/pr-workflow-subagent.md` — a short block after the Workflow list: parent states gates are green (pipeline mode, e.g. worktree-pipeline Step 10 after `/run-plan`) → skip steps 2, 2.5, 3, and 4 (gate ran per-phase upstream, docstrings filled before the gate, coverage badges would mutate the README after code review, PLAN ticked+committed; `gh pr checks` is the merge gate) and proceed via step 1 (framework detect) → step 5 (PR create) → step 6 (JIRA link); explicitly note standalone callers keep the full workflow; the block ends with the supersede sentence: "In pipeline mode this skip supersedes every other restatement of steps 2/2.5/3/4 in this file (e.g. the PLAN.md Sync section, the docstring-sweep delegation bullet, the framework quality-checks sections, the closing gates line) — those apply on the standalone path only."
    ...
    — **Done when:** the block appears verbatim-intent after the workflow list, names steps 2/2.5/3/4, names the merge gate, contains the standalone-unchanged clause, and the supersede sentence names the restating clauses (non-exhaustive, "e.g."); no text outside the added block changes
...
- [x] **1.2** Replace the Step 10 mandate parenthetical in `skills/worktree-pipeline-skill/SKILL.md` — "(its step 2.5 docstring sweep and PLAN.md sync run as part of it)" → pipeline-mode instruction: the Task prompt MUST state gates are green and instruct pr-workflow to skip its steps 2 / 2.5 / 3 / 4 (verified by run-plan per phase; CI watch below is the merge gate)
