...
- [x] **1.1** Update SKILL.md frontmatter `description` to add trigger phrases "pin agent models" / "agent model pins" while staying ≤50 words and keeping `name` unchanged
...
- [x] **1.2** Add the Step 2 extra: "Pin built-in agent models?" question payload — group multi-select (hidden maintenance `title`/`summary`/`compaction` · subagents `explore`/`general` · all five · no) + scope choice (project `<repo>/opencode.json` recommended / global `~/.config/opencode/opencode.json`) + free-form `provider/model[#variant]` input per chosen group validated to contain `/`
    ...
    — **Done:** Step 2 gained the pin extra (group multi-select incl. all five / no), a "Model-pin intake" block with the `/`-validation rule + no-hardcoded-IDs note, and the scope question (project recommended / global). files: skills/opencode-repo-setup-skill/SKILL.md; fixes: none
- [x] **1.3** Add the Step 3 write path: `{"agents": {"<id>": {"model": "..."}}}` delta through the existing mandatory jq/Node deep-merge procedure; global target = backup-then-merge of `~/.config/opencode/opencode.json`; extend the diff-check wording to `agents.*` alongside `mcp.*`; note the merge-safety rationale (agent definitions merge across layers — scalars replace, permission rules append)
    ...
    — **Done:** Step 3 gained the agents delta example, the model-only-entries-are-safe rule (scalars replace / permissions append; valid IDs enumerated; build/plan excluded), the mandatory `.bak-<timestamp>` backup for Global scope, and the generalized `mcp.*`/`agents.*` diff-check. files: skills/opencode-repo-setup-skill/SKILL.md; fixes: none
- [x] **1.4** Update the "What I do" intro (global-edit exception for this extra only) and Step 5 report/revert: list written pins (effective next session), revert = delete added `agents.<id>` keys (or whole `agents` key if we created the file), hidden agents remain non-selectable, `build`/`plan` deliberately not offered
    ...
    — **Done:** intro now states the sole exception (agent-model-pin extra, Global scope, backup-then-merge) and fixes stale `config.json` → `opencode.json`; Step 5 gained the pins-written + backup-path line, hidden-agent non-selectable note, `agents.<id>` revert wording; Governance table gained the v2 agents-docs row. files: skills/opencode-repo-setup-skill/SKILL.md; fixes: none
