...
- [x] Both files tracked and pushed, referenced from the new `_index.md` rows
- [x] `.gitignore` diff = exactly 2 negation lines
- [x] `_index.md` diff = exactly the 2 new entry blocks (17 insertions)
- [x] `git check-ignore`: the two files NOT ignored; every other `LEARNINGS/**/*.md` still ignored
- [x] bats suite green (full run — no LEARNINGS consumers expected, prove it)
...
- [x] **1.2** Full exit gate: (a) `git check-ignore -q` exits NON-zero for both new files; a control file (any other `LEARNINGS/**/*.md`) STILL ignored; (b) `.gitignore` diff vs origin/main = exactly 2 added lines, both negations; (c) `_index.md` diff = exactly 17 insertions containing the two `### ` entry headings; (d) `git add` all four artifacts → `git status --cached` shows exactly 4 paths; (e) full bats suite `bats tests/` green
    ...
    — **Done:** gate green: check-ignore 2 new NOT ignored + control ignored; numstat +2 (gitignore) / +17 (index, 2 ### headings); 4 staged paths; bats tests/ full suite 558 tests exit 0
- [x] **1.3** Commit (`docs(learnings): publish two new entries to the tracked index`), write the `tier=full` gate memo into the Trace block, tick ACs, push
    ...
    — **Done:** tier=full memo on ef9bf11; all AC ticked
...
|-------|------|--------|-------|
| 1 | full (check-ignore 3/3 + numstat +2/+17 + 4 staged paths + bats tests/ 558) | green | GATE ef9bf11 tier=full lint=t(bats) typecheck=- build=- unit=t(558/558) e2e=n.a |
