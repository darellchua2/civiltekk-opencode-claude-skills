...
- [ ] `node installer/build-registry.mjs` run and `registry.json` committed
...
- [ ] **1.1** Add `metadata.harness: "opencode"` to the 12 Tier A skills — `opencode-agent-creation`, `opencode-skill-creation`, `opencode-skills-maintainer`, `opencode-repo-setup`, `opencode-v2-migration`, `agent-introspection-debugging`, `context-budget`, `documentation-sync-workflow`, `documentation-consistency`, `strategic-compact`, `continuous-learning`, `plan-execution-skill`. Quoted string per the Mode R ruling; new `metadata:` block where absent (8 files), append sub-key where present (4 files: opencode-skills-maintainer, opencode-v2-migration, documentation-consistency, continuous-learning — plan-execution-skill to verify at edit time).
...
- [ ] **3.1** `installer/init.mjs` — after selection with target known, push warnings into the existing `sel.warnings` flow: (a) `skill.harness === "opencode"` and effective target ≠ `opencode` → cross-harness warning; (b) `skill.os.length > 0` and mapped host platform (`darwin→macos`, `win32→windows`, `linux→linux`) not in `skill.os` → OS warning. Non-fatal; prints like existing warnings.
...
- [ ] **3.2** Smoke test the AC command shape against a sandboxed HOME: `HOME=<tmp> node installer/init.mjs add opencode-v2-migration-skill --target claude --dry-run` (flags adjusted to actual CLI) — capture the warning line in the PLAN.
...
- [ ] **4.1** Full gate: bats suite + build-registry idempotency (second run → zero diff) + `git diff origin/main...HEAD -- installer/registry.json` shows only `os`/`harness` additions + count checks unchanged (34 agents / 146 skills).
