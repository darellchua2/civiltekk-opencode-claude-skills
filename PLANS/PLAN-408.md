...
- [ ] **1.2** Sharpen `skills/grilling-skill/SKILL.md:104`: name the survivor explicitly — "the canonical contract `plan-execution-skill` parses (`--soft`/`--gate`)" replacing "the execution skills parse"
...
- [ ] **2.4** Update `README.md`: 148→146 at three count sites (:15 tree, :250 `34 agents +`, :409 `All 148`); Git/Workflow category row (:582) — drop the two deleted names, count (16)→(14); append migration-history note `Post-#408: −2 execution siblings (`plan-updater-skill`, `plan-automation-loop-skill` merged into `plan-execution-skill` modes `--update`/`--gate`) → 146`. Update `opencode_app/README.md:30` 148→146
