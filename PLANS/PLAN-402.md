...
- [ ] **1.3** Write `references/validation.md`: acceptance policies per profile, the fingerprint-diff gate contract (missing dims/leaders in target = failure when source has them), the 5 dispositions (`pass`, `pass_with_warnings`, `needs_review`, `blocked`, `fail`) with trigger conditions, and the pixel-comparison prohibition
