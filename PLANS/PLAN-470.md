...
- [x] **1.1** Add plan constants + `build_plan()`: pure planner — sets PLAN_MODE, fills PLAN_STEPS per the mode table (preconditions as explicit steps: skills-only deps-check, models/migrate node-check; manifest-update NON-critical per #379's warn-and-continue, init.mjs:1160 die(2) + setup.sh warn+exit-0 contract), calls validate_mode_conflicts
    ...
    — **Done:** build_plan with validate_mode_conflicts inside; per-mode precondition steps per the table; PLAN_MODE computed; files: deploy/setup.sh; fixes: none
- [x] **1.2** Add `run_plan()`: iterate PLAN_STEPS, call each function bare, branch on criticality (non-critical fail → `log_warn` + continue; critical fail → record + stop), return failures
...
- [x] **1.4** Extract inline step code into callable functions, placed BELOW deploy_agents (preserving deploy_delegate.bats' line-order pin): `validate_opencode_install()` (from deploy_skills_only), `check_dependencies` already a function (referenced as a step), `resolve_models_config_only()` + `update_manifest()` (models-only block; update_manifest keeps the #379 warn-and-continue inside), `run_migration_only()` (migrate-only block, folding the AGENTS_DEST_DIR pre-mkdir). deploy_skills_only becomes a thin wrapper kept for its bats consumers, or re-pinned — per the map
    ...
    — **Done:** node_required, check_dependencies_strict, validate_opencode_install, resolve_models_config_only, update_manifest (dry_arg literals verbatim), run_migration_only extracted below deploy_agents (deploy_delegate order pin intact — verified by suite); deploy_skills_only thin-wrapped keeping parity pins; files: deploy/setup.sh; fixes: two degenerate newline-joining edits during insertion (provider-models line, learnings log line) — caught by bash -n and repaired immediately
- [x] **1.5** Class sweep (#467/#469 family) over the EXTRACTED code: gate the legacy-config `mv`s (setup.sh:2532/:2535) and run_migration's branch-guarded writes (:3283/:3285/:3299) per the run_cmd pattern; sweep shapes must include variable indirection, not just literal mv/mkdir strings
    ...
    — **Done:** legacy mv :2532/:2535 → run_cmd; run_migration writes verified already branch-guarded (arch-corrected list); variable-indirection shapes covered by the #467 suite + new audit; files: deploy/setup.sh; fixes: none
- [x] **1.6** Author the Phase-1-satisfiable pins NOW (before 2.1): plan-shape pins per mode, executor continue/stop/exit pins (stub functions), conflict die pin, D2 functional pins — into tests/test_plan_executor.bats; run them green against Phase 1 alone
    ...
    — **Done:** tests/test_plan_executor.bats authored and green against Phase 1 (plan shapes, executor stubs, conflict die, D2 functional pins both directions + no-config case); fixes: bats run+env-prefix 127 bug (HOME moved inside bash -c)

### Phase 2: main() restructure
- [x] **2.1** Rewrite main(): parse_arguments → validate_enable_pack → **build_plan (fail-fast validation pass)** → header (keyed off PLAN_MODE) → [network check / auto-update / TTY-gated menu — gated on the SAME flag conditions as today; menu options only SET flags] → **build_plan (rebuild after menu)** → run_plan → uniform epilogue → truthful exit. Remove all **seven** early-exit blocks (rollback :4212, update-only :4218, models-only :4226, peonping-only :4259, migrate-only :4272, skills-only :4287, check-update-only :4301) + duplicated step calls; check-update becomes a single-step plan mode (check_for_updates_only, critical); headless no-TTY notice sets SKILLS_ONLY; keep the AUTO_ACCEPT-gated "Press Enter to exit..."
    ...
    — **Done:** main rewritten — two-pass build_plan, PLAN_MODE-keyed header, gates preserved at historical reachability (network/auto-update full-only), menu options set flags only, headless notice sets SKILLS_ONLY, uniform epilogue, truthful exit; zero || true step calls; files: deploy/setup.sh; fixes: none
- [x] **2.2** Verify setup.ps1 untouched, README unchanged; PR body discloses: (a) behavior changes — models-only resolver failure now stops before manifest update; update/peonping/migrate/**check-update** failures flip exit 0 → truthful 1; headless -y over existing config now PRESERVES user content (D2) — **bash only**; (b) D2 Windows divergence (setup.ps1:1856) + models-only/migrate stock-base deferred, both tracked
    ...
    — **Done:** ps1 + README untouched (diff scope = setup.sh + 2 test files); PR disclosures prepared (see body); files: —; fixes: none

### Phase 3: end-to-end pins + full gate
- [x] **3.1** Complete tests/test_plan_executor.bats with the post-restructure pins: per-mode dry-run execution traces (steps in plan order, truthful exit, headless `--skills-only` failing-network → exit 0); D2 already pinned in 1.6 — verify still green against rewritten main
    — **Why:** The end-to-end matrix is the contract's final proof.
    — **Done when:** full file green.
    — **Consumers affected:** CI.
    — **Done:** 13/13 incl. per-mode dry-run trace order pin (skills-only) + truthful-exit pin (models-only, node absent → non-zero, message pinned); files: tests/test_plan_executor.bats; fixes: none
- [x] **3.2** Full gate: `bash -n`, `bats tests/`, `node --test tests/*.test.ts`; diff scope = setup.sh + test files only
    — **Why:** Gate contract.
    — **Done when:** all green.
    — **Consumers affected:** none.
    — **Done:** bash -n ok; bats 446 ok / 0 fail (433 + 13); node --test 30/0; all structural pins green (parity body, dry_arg literals, deploy_delegate order); files: —; fixes: none
...
|---|---|
| full (default/menu 3) | check_dependencies(C) → setup_github_cli, setup_zai_api_key, setup_nvm, setup_nodejs, setup_opencode (all non-C) → setup_config(C) → setup_local_llm → setup_vllm → setup_model_provider → deploy_agents(C) → deploy_plugins(C) → setup_opencode_init_symlink → setup_learnings_dir → setup_shell_vars |
| quick (`-q`/menu 1) | check_dependencies(C) → setup_config(C) → setup_local_llm → setup_vllm → setup_model_provider → deploy_agents(C) → deploy_plugins(C) → setup_opencode_init_symlink → setup_learnings_dir → setup_shell_vars (no env steps) |
...
| check-update (`-C`) | check_dependencies(C) → check_for_updates_only(C) |
...
GATE (fix-round head) lint=- typecheck=- build=- unit=t e2e=n.a.  (bash -n ok; bats 446 ok / 0 fail incl. 13 new plan pins; node --test 30 pass / 0 fail; scope = deploy/setup.sh + tests/test_plan_executor.bats only; arch rounds 1-2 amendments applied; structural consumers re-verified green)
