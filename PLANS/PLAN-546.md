...
## Technical Notes
- Upstream sources: Anthropic `frontend-design` skill (repo `anthropics/skills`, path `skills/frontend-design`, Apache-2.0 via its bundled LICENSE.txt; the `anthropics/claude-code` plugins copy is an unlicensed mirror) for design guidance; Vercel `react-best-practices` skill (vercel-labs/agent-skills, react-best-practices branch) for the perf ruleset. Per the AC8 ruling, provenance is per-file: frontend-design-skill cites Anthropic only; react-best-practices-skill cites Vercel only.
...
GATE review-fix tier=full lint=n.a. typecheck=n.a. build=n.a. unit=t e2e=n.a.  # Step 9 re-gate on the fixed tree — 7 suites, 0 failures; fixes: provenance repointed to anthropics/skills (Apache-2.0 per LICENSE.txt, claude-code noted as mirror only), references header 39→43, opencode_app/README 146→147
```
