...
**Pipeline mode** (parent states gates are green — e.g. worktree-pipeline Step 10 after `/run-plan`): skip steps 2, 2.5, and 4 — the gate ran per-phase upstream, docstrings were filled before the gate, and the PLAN is ticked and committed; CI (`gh pr checks`) is the merge gate. Proceed via step 1 (framework detect) → step 5 (PR create) → step 6 (JIRA link). Standalone callers (direct "create pr" without a green-gates assertion) keep the full workflow. In pipeline mode this skip supersedes every other restatement of steps 2/2.5/4 in this file (the PLAN.md Sync section, the docstring-sweep delegation bullet, the closing gates line) — those apply on the standalone path only.
