...
> **Interim workaround (#481):** 26 reviewer-baseline skills are temporarily primary-visible in lean because opencode v2.0.11 ignores agent-frontmatter `skill` allows in child sessions ([upstream anomalyco/opencode#50149](https://github.com/anomalyco/opencode/issues/50149)) — config-layer allows are the only working path. The 28 non-reviewer agents' frontmatter skill allows remain non-functional under lean until the upstream fix (this note is the deferral record). Revert: remove the entries from `deploy/skill-profiles.json` `lean` + redeploy.
